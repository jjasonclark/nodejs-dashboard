...
var TIME_LABEL_PATTERN = constants.TIME_LABEL_PATTERN;
...
MetricsProvider.prototype.getAvailableTimeRange = function getAvailableTimeRange() {
  return {
    minTime: {
      label: utils.getTimeIndexLabel(0, this.lowestAggregateTimeUnits),
      value: 0
    },
    maxTime: {
      label: utils.getTimeIndexLabel(
...
MetricsProvider.prototype.gotoTimeValue = function gotoTimeValue(timeValue) {
  // set a goto offset
  this.gotoOffset = -utils.convertElapsedTimeToTimeIndex(timeValue, 0, +this.zoomLevelKey);
  this.emit("refreshMetrics");
};
  ...
  function aggregateMetrics(currentTime, metricData) {
    ...
    var processRow = function processRow(rowIndex, rows) {
      ...
      var currentTimeIndex =
        utils.convertElapsedTimeToTimeIndex(currentTime, this._startTime, +aggregateKey);
...
MetricsProvider.prototype.getXAxis =
  function getXAxis(limit) {
    ...
    for (
      ...
      xAxis.push(utils.getTimeIndexLabel(timeIndex, +this.zoomLevelKey));
    }
...
MetricsProvider.prototype.validateTimeLabel =
  function validateTimeLabel(label) {
    ...
    timeValue = utils.convertTimeLabelToMilliseconds(label);
